...
class AmortizationRate:
    ...
    def get_amort_cost_by_name(self, cost_type):
        value = self.Mapping_dictionary[self.CostGroup][cost_type]
        return float(value)


print("Alpha_gamma cost rate is: " + str(AmortizationRate(contract_nr=124).get_amort_cost_by_name("alpha_gamma")*100) + "%")
print("Alpha_beta cost rate is: " + str(AmortizationRate(contract_nr=124).get_amort_cost_by_name("alpha_beta")*100) + "%")
