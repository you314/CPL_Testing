...
class PresentValues:
    ...
    time-improvement changes:
    -Created variables npx to not recalculate in every iteration (when possible)
    -Defined v vector v() @init; to not recalculate
    -Translated some definitions into list comprehension definitions
    """
    def __init__(self):
    ...
    def v(self) -> list[float]:
        ...
        v_vector = [(1 / (1 + i[j])) for j in range(0, len(i))] #list comprehension
        '''
        v_vector = []
    ...
    def n_m_a_x(self, deferment_period: int, m: int, age: int, birth_date: int) -> float:
        ...
        for count in range(0, m+1):
            inner_product = 1.
            npx = self.n_p_x(n=count, age=age + deferment_period, birth_date=birth_date)
            for k in range(deferment_period, deferment_period + count+1):
                inner_product *= self.v[k]
            inner_probability = npx
    ...
    #c38a and c38b both compute x_p_x(payment_duration,age,birth_date); Save running time by computing once and inserting as variable npx
    def c38a(self,payment_duration,age,birth_date, npx =0):
        print("c38a")
        if (npx == 0):
            npx = self.n_p_x(n=payment_duration, age=age, birth_date=birth_date)
        term =1
        for j in range(payment_duration-1):
            term =term + npx * self.v[j] #use npx instead of calculating n_p_x and v() for every year
            #term =term +  self.n_p_x(n=payment_duration, age=age, birth_date=birth_date) * self.v()[j]
        return term

    def c38b(self,payment_duration,age,birth_date,npx =0):
        print("c38b")
        if (npx == 0):
            npx = self.n_p_x(n=payment_duration, age=age, birth_date=birth_date)
        term1 =0
        for j in range(payment_duration-1):
            term1 = term1 + npx * self.v[j] #see c38a
    ...
    def c44(self,payment_duration,age,birth_date,deferment_period):
        term1 =1
        npx = self.n_p_x(n=deferment_period - payment_duration, age=age, birth_date=birth_date)
        for j in range(deferment_period-payment_duration-1):
            term1 = npx * self.v[j+payment_duration]
