...
class Tariff():
        ...
        def barwert_testing(self) -> float:
            ...
            c4 = self.present_values.c4_nag_k(age=self.contract_Dto.actuarial_age(),birth_date=self.contract_Dto.birth_year(), deferment_period=self.contract_Dto.deferment_period(), guarantee_time=self.contract_Dto.guarantee_time(),payment_contributions_frequency=self.contract_Dto.payment_contributions_frequency())
            c7 = self.present_values.c7_ngax_k(deferment_period=self.contract_Dto.deferment_period(),guarantee_time=self.contract_Dto.guarantee_time(), payment_frequency=self.contract_Dto.payment_contributions_frequency(), age=self.contract_Dto.actuarial_age(),birth_date=self.contract_Dto.birth_year())
            c38a = self.present_values.c38a(payment_duration=self.contract_Dto.premium_payment_duration(),
                                               age=self.contract_Dto.actuarial_age(),
                                               birth_date=self.contract_Dto.birth_year())

            e31a = self.Cost_flags.e_31a_Rxnt(deferment_period=self.contract_Dto.deferment_period(), age=self.contract_Dto.actuarial_age(),
                                               birth_date=self.contract_Dto.birth_year(), payment_duration=self.contract_Dto.premium_payment_duration())
            # Te15= self.Cost_flags.e_30(payment_duration=self.contract_Dto.premium_payment_duration(),max_provizionsbezug=30)
            # #Te45= self.Cost_flags.e_45()
            # #Te46 = self.Cost_flags.e_46()
            # Te30 =self.Cost_flags.e_31a_Rxnt(deferment_period=self.contract_Dto.deferment_period(),age=self.contract_Dto.actuarial_age(),birth_date=self.contract_Dto.birth_year(),payment_duration=self.contract_Dto.premium_payment_duration())
            ...
            # Result = 1000 * num/den
            end_time = time.time()
            return c0, c1, c2, c3, c4, c7, c38a, e31a, end_time - start_time


c0, c1, c2, c3, c4, c7, c38a, e31a, timing = Tariff().barwert_testing()
...
print("c2 for tariff pv example: " + str(c2) + " (Desired value: 26.141518) " + "in " + str(timing) + " seconds")
print("c3 for tariff pv example: " + str(c3) + " (Desired value: 4.681005) " + "in " + str(timing) + " seconds")
print("c4 for tariff pv example: " + str(c4) + " (Desired value: 2.70532) " + "in " + str(timing) + " seconds")
print("c7 for tariff pv example: " + str(c7) + " (Desired value: 12.69427175) " + "in " + str(timing) + " seconds")
print("c38a for tariff pv example: " + str(c38a) + " (Desired value: 15.622082) " + "in " + str(timing) + " seconds")
print("e31a for tariff pv example: " + str(e31a) + " (Desired value: 0.05) " + "in " + str(timing) + " seconds")
